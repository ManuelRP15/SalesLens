...
| `duplicate-detection.ts` | `computeDuplicateClusters()` — pure Translation Health QA check: per-language clusters of 2+ distinct elements sharing a normalized value (`DECISIONS.md #64`). Kept as its own dependency-free module (not inside `background/index.ts`) precisely so it IS unit-testable, unlike `computeTranslationHealth`. |
| `duplicate-detection.test.ts` | Unit tests for the above — no Chrome/Salesforce dependency. |
